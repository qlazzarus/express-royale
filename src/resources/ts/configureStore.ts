...
import {alertMiddleware, requestMiddleware, responseMiddleware} from '@/middlewares';
...
export default (client: AxiosInstance) => {
    ...
    const enhancers = process.env.NODE_ENV === 'production' ?
        compose(applyMiddleware(thunk, logger, alertMiddleware(), requestMiddleware(client), responseMiddleware())) :
        composeEnhancers(applyMiddleware(thunk, alertMiddleware(), requestMiddleware(client), responseMiddleware()));
